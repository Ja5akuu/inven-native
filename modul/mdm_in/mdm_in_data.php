		...
		if($hapus){	
			...
			while ($itmRow	= mysqli_fetch_assoc($itmQry)) {
				$update=mysqli_query($koneksi,"UPDATE ms_barang SET stok_barang=stok_barang-$itmRow[jumlah_in] 
					WHERE id_barang='".$itmRow['id_barang']."'", $koneksi) 
				or die ("Gagal kosongkan tmp".mysqli_error());
			}
			...
			$_SESSION['pesan'] = 'Data penerimaan modem berhasil dihapus';
			echo '<script>window.location="?page=dtmdmin"</script>';
		}
...
<form action="<?php $_SERVER['PHP_SELF']; ?>" method="POST">
	<div class="portlet box blue">
		<div class="portlet-title">
			<div class="caption"><span class="caption-subject uppercase bold hidden-xs">Data Stok Masuk Barang</span></div>
			<div class="actions">
				<a href="?page=addmdmin" class="btn blue"><i class="icon-plus"></i></a>	
				...
				<tbody>
					...
					while ($data = mysqli_fetch_array($dataQry)) {
						...
						<tr class="odd gradeX">
							...
							<td><div align="center"><a href="?page=dtlmdmin&amp;id=<?php echo base64_encode($Kode); ?>" class="btn btn-xs green"><i class="icon-book-open"></i></a></div></td>
						</tr>
					<?php } ?>
				</tbody>
			</table>
